    ...
    def forward(self, x: Tensor, padding_mask: Tensor, U: Tensor) -> Tensor:
        ...
        for layer in self.encoder:
            if self.gradient_checkpointing and self.training:
                def _layer_forward(x_in: Tensor, _layer=layer, _padding_mask=padding_mask, _U=U) -> Tensor:
                    return _layer(x_in, _padding_mask, _U)

                x = torch_checkpoint(_layer_forward, x, use_reentrant=False)
            else:
