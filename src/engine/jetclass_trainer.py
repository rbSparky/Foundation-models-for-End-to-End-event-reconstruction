    ...
    def train(self) -> Tuple[Dict[str, List[float]], nn.Module]:
        try:
            ...
            for epoch in range(self.start_epoch, self.num_epochs):
                epoch_t0 = self.profiler.begin_epoch(self.device)
                ...
                for batch_idx, (X, y) in enumerate(self.train_loader):
                    ...
                    sched_stride = max(int(total_steps * 0.3 // 15), 1)
                    if self.scheduler and (step > total_steps * 0.7 and step % sched_stride == 0):
                            self.scheduler.step()
                ...
                # Save best model according to configured monitor.
                if self.best_model_path and self.rank == 0:
                    monitor_values = {
                        "val_loss": val_loss,
                        "val_metric": val_metric,
                    }
                    current_score = monitor_values.get(self.best_monitor, val_loss)
            ...
            self.profiler.maybe_save(self.rank)
